...
pacman::p_load(writexl, readxl, tidyverse, lubridate, here, stringi)
...
filename <- "UTF8_Export"

input_file_path <- paste0("input\\", filename,".csv")
WSDB_data <- readr::read_csv(input_file_path)

#check encodings
# Detect encodings of the raw data
raw_content <- readBin(input_file_path, what = "raw", n = 10000)# Detect the encoding
detected_encodings <- stringi::stri_enc_detect(raw_content)# Print the detected encodings
# print(detected_encodings)

comments = WSDB_data%>%filter(!is.na(COMMENTS))%>%select(COMMENTS)

iconv(comments$COMMENTS, "Windows-1252", "UTF-8")
