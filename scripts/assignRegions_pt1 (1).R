...
# load input WS data
    WS_data <- read_excel(here("input", input_file), sheet = sheet)%>%select(-COMMONNAME, -URI, -SCIENTIF)%>%mutate(LATITUDE = as.numeric(trimws(LATITUDE)), 
                                                                                                                    LONGITUDE = as.numeric(trimws(LONGITUDE)))
    
# check species, add common names and scientific names based on codes
    SP_data <- read_excel(here("input", species), sheet = "WS_SPECIESCODES")
